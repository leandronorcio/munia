...
export async function PUT(request: Request, { params }: { params: { commentId: string } }) {
  ...
  try {
    ...
    const { str, usersMentioned } = await convertMentionUsernamesToIds({ str: content });
    content = str;
    ...
    // Convert Prisma result to API response
    const output: GetComment = await toGetComment(res);
